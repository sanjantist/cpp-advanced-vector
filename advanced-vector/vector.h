...
void Vector<T>::CopyFrom(const Vector<T> &rhs) {
  size_t common = std::min(rhs.size_, size_);
  std::copy(rhs.data_.GetAddress(), rhs.data_.GetAddress() + common,
            data_.GetAddress());
  std::destroy_n(data_.GetAddress() + common, size_ - common);
  std::uninitialized_copy_n(rhs.data_.GetAddress() + common, rhs.size_ - common,
...
typename Vector<T>::iterator Vector<T>::Emplace(const_iterator pos,
  ...
  assert(pos_it >= begin() && pos_it < end());
